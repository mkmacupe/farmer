...
Функция: инициализация предзаполненных данных и reset состояния перед защитой. Учебные логины и пароли перечислены в `accounts.txt`; публичный `seed-login` остаётся зависимым от `app.demo.enabled`.
...
Этот контур не является частью обычного production startup: в `application-prod.yml` публичный demo-login и startup-seed выключены по умолчанию. Сам reset доступен как защищённый manager-only endpoint, чтобы можно было восстановить учебный сценарий на задеплоенном сайте без ручного доступа к базе.
...
- backend endpoint `POST /api/scenario/reset` после авторизации менеджера;
