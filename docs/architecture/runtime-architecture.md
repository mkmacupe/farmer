...
- `DataInitializer` — базовые demo-пользователи и товары без накопленного runtime-состояния.

В результате демонстрация всегда стартует из одинакового чистого состояния: без заказов и без точек доставки.
