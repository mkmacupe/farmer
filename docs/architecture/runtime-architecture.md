...
Функция: логин, seed-login, выпуск JWT и проверка ролей.
...
Функция: инициализация предзаполненных данных и reset состояния перед защитой.
...
- role-based shell, который после логина подгружает только нужную рабочую область пользователя;
...
- backend endpoint `POST /api/scenario/reset`;
